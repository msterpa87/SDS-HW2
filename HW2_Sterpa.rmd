...
EM_simulation <- function(n, M, k_max, subdivisions = 200, verbose = TRUE) {
  ...
  for (n_sim in 1:M) {
    ...
    for (k in 1:k_max) {
      ...
      model_eval[8,k] <- tryCatch(integrate(f_abs, lower = 0, upper = 1,
...
```{r large sample simulation, message=FALSE, warning=FALSE, include=FALSE}
result.large <- EM_simulation(1000, 50, 8)
```

```{r small sample simulation, message=FALSE, warning=FALSE, include=FALSE}
result.small <- EM_simulation(100, 50, 8)
```
